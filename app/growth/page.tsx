...
const CATEGORIES = [
  { name: "All Struggles", icon: Flame, count: 0, glow: "hover:text-orange-500" },
  { name: "Hall of Fame", icon: Trophy, count: 0, glow: "hover:text-yellow-500" },
  { name: "Internships", icon: Briefcase, count: 0, glow: "hover:text-blue-500" },
  { name: "Exams", icon: FileText, count: 0, glow: "hover:text-red-500" },
  { name: "Skills", icon: Zap, count: 0, glow: "hover:text-yellow-400" },
  { name: "Career", icon: TrendingUp, count: 0, glow: "hover:text-green-500" },
  { name: "Mental Health", icon: HeartPulse, count: 0, glow: "hover:text-pink-500" },
  { name: "Other", icon: Pin, count: 0, glow: "hover:text-zinc-500" },
];
...
export default function CampusPage() {
  ...
  const [category, setCategory] = useState("All Struggles");
  ...
  useEffect(() => {
    const saved = localStorage.getItem("growthReadCounts");
    if (saved) {
  ...
  useEffect(() => {
    if (activeChat) {
      ...
      setReadCounts(prev => {
        ...
        localStorage.setItem("growthReadCounts", JSON.stringify(next));
        return next;
      });
    }
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "growthStruggles", activeChat.id, "messages"),
  ...
  const handleStartWorking = async (
    ...
    // Join the group
    try {
      const docRef = doc(db, "growthStruggles", problem.id);
      ...
      // Send System Message
      await addDoc(collection(db, "growthStruggles", problem.id, "messages"), {
        text: `${alias} joined to work on this struggle`,
        type: "system",
  ...
  const handleUpvote = async (id: string, currentVotes: number, upvotedBy: string[] = []) => {
    ...
    try {
      ...
      if (newVotes === 10) {
        toast("🔥 This struggle is trending! Be the first to solve it.", {
          duration: 5000,
          icon: "🚀",
          style: {
            background: "#111",
  ...
  const handleMarkAsSolved = async () => {
    ...
    if (!solutionText.trim()) {
      toast.error("You must explicitly provide a link to the project!");
      return;
    }

    try {
      const docRef = doc(db, "growthStruggles", problemToSolve.id);
      await updateDoc(docRef, {
        status: "solved",
      ...
      toast.success("Struggle marked as solved! Added to Hall of Fame 🏆");
  ...
  const handleSubmit = async () => {
    ...
    if (description.trim().length < 80) {
      toast.error("Please describe your struggle in more detail (min 80 chars)! 📝");
      return;
    }

    if (!audience.trim()) {
      toast.error("Who does this affect? (e.g. graduates, freelancers)");
      return;
    }
    ...
    if (duplicateOfId) {
      toast.success("Struggle already exists! Auto-upvoted it for you ❤️");
    ...
    try {
      const problemsRef = collection(db, "growthStruggles");
      ...
      toast.success("New Struggle Posted! Use group chat to overcome it. 🚀");
      setIsModalOpen(false);
      setTitle("");
      setDescription("");
      setAudience("");
  ...
  const handleDashboardSummary = async () => {
    ...
    try {
      ...
      for (const group of activeGroups) {
        const q = query(collection(db, "growthStruggles", group.id, "messages"), orderBy("createdAt", "desc"), limit(50));
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      const docRef = doc(db, "growthStruggles", activeChat.id);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, "growthStruggles", activeChat.id, "messages"), msgData);
      await updateDoc(doc(db, "growthStruggles", activeChat.id), {
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    try {
      await deleteDoc(doc(db, "growthStruggles", activeChat.id, "messages", messageId));
    } catch (e) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 25 * 1024 * 1024) {
      toast.error("File too large! Max 25MB.", { id: "upload-growth" });
      return;
    }
    ...
    try {
      ...
      if (file.type.startsWith("image/")) {
        toast.loading("Compressing image...", { id: "upload-growth" });
        const dataUrl = await compressImage(file);
        const res = await fetch(dataUrl);
        fileToUpload = await res.blob();
      }

      toast.loading("Uploading...", { id: "upload-growth" });
      ...
      uploadTask.on('state_changed', 
        ...
        (err) => {
          ...
          toast.error("Upload failed", { id: "upload-growth" });
          setPendingAttachment(null);
          setIsUploading(false);
        }, 
        async () => {
          try {
            ...
            setIsUploading(false);
            toast.dismiss("upload-growth");
            toast.success("Ready to send!", { id: "upload-growth-success", duration: 2000 });
          } catch (error) {
            console.error("Post-upload error:", error);
            setIsUploading(false);
            setPendingAttachment(null);
            toast.error("Finalizing upload failed", { id: "upload-growth" });
          }
        }
      );
    } catch (err) {
      console.error("Processing error:", err);
      toast.error("Processing failed", { id: "upload-growth" });
      setPendingAttachment(null);
      setIsUploading(false);
    }
  };
  ...
  const filteredProblems = selectedCategory === "All Struggles"
      ...
      <header className="h-[72px] bg-[#0a0a0a] border-b border-zinc-900 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-40 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl sm:text-[22px] font-bold text-white tracking-tight leading-tight">Growth Mode</h1>
            <span className="text-[10px] sm:text-xs text-gray-400">Level up your skills & career</span>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-xl mx-4 sm:mx-8 hidden md:block relative">
          ...
          <input
            type="text"
            placeholder="Search struggles, projects, or mentors..."
      ...
      <div className="flex flex-1 overflow-hidden h-[calc(100vh-72px)] relative">
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(count > 0 || cat.name === "All Problems" || cat.name === "All Struggles") && (
                      <div className="flex items-center gap-2">
                        <div className={`w-1.5 h-1.5 rounded-full ${
                          cat.name === "All Struggles" || cat.name === "All Problems" ? "bg-orange-500" :
                          cat.name === "Hall of Fame" ? "bg-yellow-500" :
                          cat.name === "Internships" ? "bg-blue-500" :
                          cat.name === "Exams" ? "bg-red-500" :
                          cat.name === "Skills" ? "bg-yellow-400" :
                          cat.name === "Career" ? "bg-green-500" :
                          cat.name === "Mental Health" ? "bg-pink-500" :
                          "bg-zinc-500"
                        }`} />
              ...
              <div className="bg-[#111111] border border-green-500/20 rounded-2xl p-5 mb-6 shadow-xl shadow-green-500/5">
                ...
                <div className="flex flex-col gap-2">
                  {(() => {
                    ...
                    if (topSolvers.length === 0) return <div className="text-xs text-gray-500 italic px-2">No growth projects built yet this week. Be the first!</div>;
            ...
            {filteredProblems.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 px-6 border border-dashed border-zinc-800 rounded-3xl bg-zinc-900/10">
                {selectedCategory === "Hall of Fame" ? (
                  ...
                  <div className="text-center">
                    <span className="text-6xl mb-6 block">🌱</span>
                    <h3 className="text-xl font-bold text-white mb-2">No Struggles Found</h3>
                    <p className="text-gray-500 mb-6 font-medium">No struggles posted in "{selectedCategory}" yet.</p>
                    <button
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center flex-wrap gap-2 mb-4">
                      ...
                      {['growth', p.category.toLowerCase().replace(/ /g, "")].map(tag => (
                    ...
                    {p.status === "solved" && p.solution && (
                      <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-4 mb-5">
                        <div className="text-[11px] font-bold text-cyan-400 mb-1.5 uppercase tracking-wider">Solution / Growth:</div>
                        ...
                        {p.status !== "solved" ? (
                          <>
                            <button
                              ...
                              className="flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-green-500/20 text-green-500 hover:bg-green-500/10 transition text-[11px] font-black uppercase tracking-tight"
                            >
                              ✅ Overcome
                            </button>
                            <button
                              onClick={() => handleUpvote(p.id, p.votes, p.upvotedBy || [])}
                              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full transition text-[11px] font-black uppercase tracking-tight ${p.votes >= 10 ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'border border-pink-500/20 text-[#f01c7d] hover:bg-pink-500/10 bg-zinc-900'}`}
                            >
        ...
        {activeChat && (
          <div className={`absolute top-0 right-0 h-full pb-safe ${isChatExpanded ? 'w-full lg:w-[calc(100%-260px)]' : 'w-full sm:w-[400px] lg:w-[450px]'} bg-[#0a0a0a] border-l border-zinc-900 shadow-2xl flex flex-col z-20 transition-all duration-300 transform`}>
            ...
            {/* Pinned Struggle Card (Replaces banner) */}
              ...
              <div className="px-4 flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h4 className="text-xs font-bold text-white transition-all truncate">Active Struggle</h4>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold uppercase tracking-tighter ${
                      ...
                      {activeChat.status === "solved" ? "Overcome" : activeChat.status}
                    </span>
                  </div>
              ...
              <div className={`transition-all duration-300 border-l border-zinc-900 bg-[#0a0a0a] overflow-y-auto shrink-0 ${isMemberListOpen ? 'w-[200px] sm:w-[240px]' : 'w-0'}`}>
                <div className="p-4 pt-6">
                  ...
                  <div className="space-y-6">
                    {/* Role: Struggle Poster */}
                    <div>
                      <h5 className="text-[10px] text-zinc-600 font-bold mb-3 px-1 uppercase tracking-tighter italic">Struggle Poster</h5>
                          ...
                          if (poster && poster.status === 'online') {
                            return (
                              <div className="flex items-center gap-2.5 p-1.5 rounded-lg hover:bg-zinc-900/50 transition">
                                <div className="relative">
                                  <div className="w-7 h-7 rounded-full bg-cyan-600 flex items-center justify-center text-[10px] font-bold text-white uppercase">
                                ...
                                <span className="text-xs font-medium text-gray-300 truncate">{poster.alias}</span>
                              </div>
                            );
                          }
              ...
              <div className="space-y-6 min-h-[300px] flex flex-col justify-center">
                {guidedStep === 1 && (
                  <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                    <label className="block text-[10px] font-black text-zinc-500 mb-4 uppercase tracking-[0.2em]">Step 1: The Breakthrough</label>
                    <div className="space-y-5">
                      <div>
                        <label className="block text-xs font-bold text-gray-400 mb-2">What's the struggle?</label>
                        <input
                          type="text"
                          value={title}
                          onChange={(e) => setTitle(e.target.value)}
                          placeholder="e.g. Scaling my SaaS project"
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {CATEGORIES.filter(c => c.name !== 'All Struggles' && c.name !== 'Hall of Fame').slice(0, 6).map(cat => (
                            <button
                              key={cat.name}
                ...
                {guidedStep === 2 && (
                  <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="space-y-5">
                      ...
                      <div>
                        <label className="block text-xs font-bold text-gray-400 mb-2">Who else faces this?</label>
                        <input
                          type="text"
                          value={audience}
                          onChange={(e) => setAudience(e.target.value)}
                          placeholder="e.g. Solo founders, Frontend devs"
                          className="w-full px-4 py-3.5 rounded-xl bg-[#0a0a0a] border border-zinc-800 focus:border-cyan-500 outline-none text-white placeholder:text-zinc-700 transition-all text-sm"
                        />
                      </div>
                    </div>
              ...
              <div className="flex-1 p-6 space-y-4 overflow-y-auto custom-scrollbar">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Skill Sharing Session"
                ...
                <div>
                  <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 px-0.5">Link to Project</label>
                  <select
          ...
          <div className="bg-[#111111] border border-zinc-800 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 h-[600px] flex flex-col">
            <div className="p-6 border-b border-zinc-900 flex items-center justify-between bg-[#111]">
              <div className="flex bg-zinc-900/50 p-1 rounded-xl">
                <button onClick={() => setMeetupTab('upcoming')} className={`px-4 py-2 rounded-lg text-xs font-black uppercase transition-all ${meetupTab === 'upcoming' ? 'bg-zinc-800 text-white shadow-lg' : 'text-zinc-500 hover:text-gray-300'}`}>Upcoming</button>
                <button onClick={() => setMeetupTab('past')} className={`px-4 py-2 rounded-lg text-xs font-black uppercase transition-all ${meetupTab === 'past' ? 'bg-zinc-800 text-white shadow-lg' : 'text-zinc-500 hover:text-gray-300'}`}>Archive</button>
              </div>
              <button onClick={() => setIsCalendarOpen(false)} className="text-gray-400 hover:text-white transition-colors bg-white/5 p-2 rounded-xl"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              {(() => {
                ...
                if (filtered.length === 0) {
                  return (
                    <div className="h-full flex flex-col items-center justify-center text-center p-12 space-y-4">
                      <div className="w-20 h-20 bg-zinc-900 rounded-full flex items-center justify-center"><CalendarIcon className="w-10 h-10 text-zinc-700" /></div>
                      <div className="space-y-1">
                        <h3 className="text-white font-bold">No {meetupTab} meetups</h3>
                        <p className="text-xs text-zinc-500 max-w-[200px]">Collaborate with others to grow faster!</p>
                      </div>
                      <button onClick={() => { setIsCalendarOpen(false); setIsMeetupModalOpen(true); }} className="py-2.5 px-6 bg-gradient-to-r from-[#f01c7d] to-[#7c3aed] text-white text-[11px] font-black uppercase rounded-full shadow-lg">Plan Meetup</button>
                    </div>
                  );
                }
                ...
                return filtered.map((m: any) => {
                  ...
                  let borderClass = "border-l-cyan-500"; let bgClass = "bg-cyan-500/5"; let Icon = Zap; let textColor = "text-cyan-400";
                  if (m.category === 'Career') { borderClass = "border-l-purple-500"; bgClass = "bg-purple-500/5"; Icon = TrendingUp; textColor = "text-purple-400"; }
                  else if (m.category === 'Exams') { borderClass = "border-l-yellow-500"; bgClass = "bg-yellow-500/5"; Icon = FileText; textColor = "text-yellow-400"; }
                  else if (m.category === 'Skills') { borderClass = "border-l-pink-500"; bgClass = "bg-pink-500/5"; Icon = Zap; textColor = "text-pink-400"; }
                      ...
                      <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                             <div className={`p-1.5 rounded-lg ${bgClass} ${textColor}`}><Icon className="w-3.5 h-3.5" /></div>
                          ...
                          <div className="flex flex-wrap gap-3 items-center">
                            <span className="text-[11px] text-gray-500 flex items-center gap-1.5 bg-zinc-900/50 px-2 py-1 rounded-lg"><MapPin className="w-3 h-3" /> {m.location}</span>
                            <div className="flex items-center -space-x-2">
                              {m.attendees?.slice(0, 3).map((at: string, idx: number) => (<div key={idx} className="w-6 h-6 rounded-full bg-zinc-800 border-2 border-black flex items-center justify-center text-[8px] font-black text-white">{idx+1}</div>))}
                              {m.attendees?.length > 3 && <div className="w-6 h-6 rounded-full bg-zinc-900 border-2 border-black flex items-center justify-center text-[8px] font-black text-gray-500">+{m.attendees.length-3}</div>}
                        ...
                        <div className="flex flex-row md:flex-col items-center gap-4">
                          <div className="bg-[#111] border border-zinc-800 rounded-2xl px-5 py-3 text-center min-w-[100px] shadow-xl">
                            <div className="text-xs font-black text-gray-500 uppercase tracking-tighter mb-0.5">{new Date(m.date).toLocaleDateString([], { month: 'short' })}</div>
                            <div className="text-2xl font-black text-white leading-none">{new Date(m.date).toLocaleDateString([], { day: 'numeric' })}</div>
                            <div className={`text-[10px] font-bold mt-1.5 ${textColor}`}>{m.time}</div>
                          </div>
                          {meetupTab === 'upcoming' ? (
                            <button onClick={() => handleJoinMeetup(m.id, m.attendees)} className={`w-full py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isAttendee ? 'bg-zinc-800 text-zinc-400 border border-zinc-700' : 'bg-white text-black hover:bg-gray-200'}`}>{isAttendee ? 'Joined' : 'Join'}</button>
                          ) : (
                             m.outcome ? <div className="px-3 py-1.5 bg-green-500/10 border border-green-500/20 rounded-full flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3 text-green-500" /><span className="text-[9px] font-black text-green-400 uppercase">Success</span></div> : isCreator && (
                               <button onClick={() => { const text = prompt("What was the breakthrough in this meetup?"); if (text) handlePostOutcome(m.id, text); }} className="py-2.5 px-4 bg-cyan-600 hover:bg-cyan-500 text-white text-[10px] font-black uppercase rounded-xl transition-all shadow-lg shadow-cyan-900/40">Post Outcome</button>
                             )
                          )}
                        </div>
                      </div>
                      {m.outcome && <div className="px-5 pb-5 pt-0 border-t border-zinc-900/50 mt-1"><div className="bg-zinc-900/30 p-3 rounded-xl border border-zinc-800/50"><div className="flex items-center gap-2 mb-1.5"><Sparkles className="w-3 h-3 text-yellow-500" /><span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Breakthrough</span></div><p className="text-[11px] text-gray-400 leading-relaxed italic">"{m.outcome}"</p></div></div>}
                    </div>
                  );
                });
              })()}
              ...
              <div>
                ...
                <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-xl">
                  <p className="text-xs text-green-400 flex items-start gap-2">
                    ...
                    <span>By marking this overcome, you will be permanently credited as the builder for this struggle on your profile. Awesome job closing the loop!</span>
                  </p>
                </div>
              </div>
              <div className="pt-2">
                <button
                  ...
                  className="w-full bg-gradient-to-r from-[#f01c7d] to-[#7c3aed] hover:from-[#d81970] hover:to-[#6d28d9] text-white font-bold py-3.5 rounded-xl shadow-[0_0_20_rgba(240,28,125,0.3)] transition-all flex justify-center items-center gap-2 text-sm hover:-translate-y-0.5"
                >
                  Submit Solution & Mark Overcome
                </button>
              </div>
            </div>
          </div>
